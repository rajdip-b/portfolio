	...
	return (
		...
		>
			<Image
				...
				width={70}
				height={70}
				src={require(`../../assets/images/${stack.logo}`)}
			...
			<div className={'flex gap-3 cursor-pointer my-5 w-full flex-wrap items-center justify-center'}>
				{stack.skills.map((skill, index) => (
					<AnimationOnScroll key={index} animateIn={'animate__fadeInRight'} animateOnce delay={index * 200}>
						<Skill skill={skill} />
					</AnimationOnScroll>
				))}
			</div>
